...
3. In SPEC.md change `Phase: pre-alpha` to `Phase: hardening`. In `./prove.sh` change
   `PROFILE=lite` to `PROFILE=strict`.
...
5. Extend `./prove.sh` for the new phase: the real test suite, lint, typecheck, and a secrets
   check, so the gate matches what "done" now means. Run it; it must still pass.
